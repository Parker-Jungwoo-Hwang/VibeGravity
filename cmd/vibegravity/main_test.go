// ============================================================
// FILE     : cmd/vibegravity/main_test.go
...
func TestBuiltVibegravityBinarySmoke(t *testing.T) {
	t.Parallel()

	root := filepath.Clean(filepath.Join("..", ".."))
	bin := filepath.Join(t.TempDir(), "vibegravity")
	build := exec.Command("go", "build", "-ldflags", "-X main.version=test-smoke", "-o", bin, "./cmd/vibegravity")
	build.Dir = root
	if output, err := build.CombinedOutput(); err != nil {
		t.Fatalf("build vibegravity binary: %v\n%s", err, output)
	}
